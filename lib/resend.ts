...
export async function sendApprovalEmail(to: string, name: string) {
  await getResend().emails.send({
    ...
    html: `
      ...
      <a href="${APP_URL()}/login" style="background:#6C3DE0;color:#fff;padding:12px 24px;border-radius:6px;text-decoration:none;display:inline-block;margin-top:16px;">Log In Now</a>
...
export async function sendPaymentConfirmationEmail(to: string, name: string) {
  await getResend().emails.send({
    ...
    html: `
      ...
      <a href="${APP_URL()}/login" style="background:#6C3DE0;color:#fff;padding:12px 24px;border-radius:6px;text-decoration:none;display:inline-block;margin-top:16px;">Access Your Course</a>
    `,
  });
}
...
export async function sendNewStudentNotification(studentName: string, studentEmail: string) {
  await getResend().emails.send({
    ...
    html: `
      ...
      <a href="${APP_URL()}/creator/students" style="background:#6C3DE0;color:#fff;padding:12px 24px;border-radius:6px;text-decoration:none;display:inline-block;margin-top:16px;">Review Student</a>
    `,
  });
}
